...
float Logger::clampSmallCurrent(float current, float threshold) {
    return (current < threshold) ? 0.0f : current;
}

bool Logger::shouldLogMeasurements(
...
void Logger::setDisplay(Display *disp, int line_h, int val_x,
...
void Logger::initializeDisplay(float target_current_per_tec) {
    if (!display)
        return;
...
void Logger::updateDisplay(float I1, float I2, float duty,
                                  SystemState state,
...
void Logger::logCalibrationResults(float acs1_offset_V,
                                          float acs2_offset_V) {
...
void Logger::logSoftStartBegin(float target_current_per_tec,
                                      float max_duty) {
